...
import random,utime,struct                                          #   Instructor: Ivo Georgiev, Class [CPE-1040-002 38108.201930]
...
def bitvaluecreation():                                             # bitvaluecreation:
    global bitvalue,bituin,bitsin,bitflt,bitaci                     # This takes the 32 bits generated by the bit creation process,
    bitstring = (int((''.join(bits)),2))                            # then converts the binary string into the 4 values as outlined
    bitaci = struct.pack('<I',bitstring)                            # in the project objectives. The 4 values are unsigned / signed
    bituin = int(''.join(bits),2)                                   # integers, the float32 interpretation, and 4 ascii characters.
    bitsin, = struct.unpack('<i',bitaci)
    bitflt, = struct.unpack('<f',bitaci)
def cursor():                                                       # cursor:
...
while True:                                                         # main loop:
    ...
    if screen == 2:                                                 # selection screen:
        if y == 0:                                                  # Screen 2 is the selection screen, where the values of 32 bits
            display.show(Image('05105:05105:05105:05105:00550'))    # defined by the user will be displayed in different variations
            if button_a.is_pressed():                               # as they determine and select between various interpretations.
                display.scroll(bituin, delay=100)                   # The menu displays each value definition by alphabetic symbol.
        elif y == 1:
            display.show(Image('00555:05100:00550:00015:05550'))
            if button_a.is_pressed():
                display.scroll(bitsin, delay=100)
        elif y == 2:
            display.show(Image('05555:05100:05550:05100:05100'))
            if button_a.is_pressed():
                display.scroll(bitflt, delay=100)
        elif y == 3:
            display.show(Image('00550:05101:05100:05101:00550'))
            if button_a.is_pressed():
                display.scroll(bitaci, delay=100)
        if button_b.is_pressed():
            while True:
                if not button_b.is_pressed():
                    break
